...
async function lookupRepo(owner, repo) {
    const [repoRes, issuesRes, pullsRes] = await Promise.all([
        ...
        fetchGitHub(`/repos/${owner}/${repo}/issues?state=open&per_page=30&sort=created&direction=asc`, {
            cacheKey: `repoIssues:${owner}/${repo}`,
            ttlMs: 2 * 60 * 1000,
        }),
        fetchGitHub(`/repos/${owner}/${repo}/pulls?state=open&per_page=30&sort=created&direction=asc`, {
            cacheKey: `repoPulls:${owner}/${repo}`,
            ttlMs: 2 * 60 * 1000,
        }),
    ]);

    /* issues endpoint also returns PRs — filter them out; pulls come from
       the dedicated pulls endpoint so counts are accurate */
    const slim = (i) => ({
        number: i.number,
        title: i.title || "",
        state: i.state || "open",
        ...
        user: i.user?.login || "",
        html_url: i.html_url || `https://github.com/${owner}/${repo}/issues/${i.number}`,
        labels: (i.labels || []).map((l) => (typeof l === "string" ? l : l.name)).filter(Boolean),
    });

    const issues = (issuesRes.data || []).filter((i) => !i.pull_request).map(slim);
    const pulls = (pullsRes.data || []).map((p) => ({ ...slim(p), isPr: true }));
    const all = [...issues, ...pulls].sort((a, b) => new Date(a.created_at) - new Date(b.created_at));
    ...
    return {
        ...
        cached: repoRes.cached && issuesRes.cached && pullsRes.cached,
        rateLimit: repoRes.rateLimit,
    };
}
